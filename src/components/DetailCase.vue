<template>
	<div>
		...
		<el-table :data="list" v-loading="listLoading" border style="width:100%" @selection-change="handleSelectionChange">
			...
			<el-table-column prop="thumb" label="缩图(200 X 450)" min-width="150">
			</el-table-column>
			<el-table-column prop="pic" label="原图(182 X 245)" min-width="150">
			</el-table-column>
		...
		<el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editFormData" label-width="80px" :rules="formRules" ref="editForm">
				...
				<el-form-item label="名称" prop="name">
					<el-input v-model="editFormData.name" placeholder="详情名称，小于30个字符"></el-input>
				</el-form-item>
				<el-form-item label="缩图" prop="thumb">
					<el-input v-model="editFormData.thumb" placeholder="图片尺寸(200 X 450)"></el-input>
				</el-form-item>
				<el-form-item label="原图" prop="pic">
					<el-input v-model="editFormData.pic" placeholder="图片尺寸(182 X 245)"></el-input>
				</el-form-item>
				<el-form-item label="详情" prop="detail">
					<el-input v-model="editFormData.detail" type="textarea" :rows="10" placeholder="必须切片且每张切片的宽度必须大于高度，多张图片使用 | 分隔，图片格式为gif，jpg，jpeg，png的一种"></el-input>
				</el-form-item>
				<el-form-item label=" ">
		...
		<el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addFormData" label-width="80px" :rules="formRules" ref="addForm">
				<el-form-item label="名称" prop="name">
					<el-input v-model="addFormData.name" placeholder="详情名称，小于30个字符"></el-input>
				</el-form-item>
				<el-form-item label="缩图" prop="thumb">
					<el-input v-model="addFormData.thumb" placeholder="图片尺寸(200 X 450)"></el-input>
				</el-form-item>
				<el-form-item label="原图" prop="pic">
					<el-input v-model="addFormData.pic" placeholder="图片尺寸(182 X 245)"></el-input>
				</el-form-item>
				<el-form-item label="详情" prop="detail">
					<el-input v-model="addFormData.detail" type="textarea" :rows="10" placeholder="必须切片且每张切片的宽度必须大于高度，多张图片使用 | 分隔，图片格式为gif，jpg，jpeg，png的一种"></el-input>
				</el-form-item>
				<el-form-item label=" ">
...
<script>
	...
	import {getDetailCasesList, delDetailCases, delMultiDetailCases, editDetailCases, addDetailCases} from '../api/api';

	import {isImageUrl, isImagesUrl, isUrl} from '../common/js/rule';

	export default {
		name: 'detailCase', //--组件名称
		data: function() {
			...
			return {
				...
				nowPage: 1,
				// 预览地址
				detail: 'http://www.ipinru.com/custom/detail/id/',
				// 帅选条件
				filter: {
					id: ''
				},
		...
		methods: {
			...
			getList() {
				...
				getDetailCasesList(params).then(res => {
			...
			handleDel(id) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					this.listLoading = true;
					let params = {id: id};
					delDetailCases(params).then(res => {
						this.listLoading = false;
						this.$message({
							message: '删除成功',
							type: 'success'
			...
			handleDelMulti() {
				...
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					this.listLoading = true;
					let params = {ids: ids};
					delMultiDetailCases(params).then(res => {
						this.listLoading = false;
						this.$message({
							message: '删除成功',
							type: 'success'
			...
			editSubmit(formName) {
				this.$refs[formName].validate(valid =>{
					if (valid) {// 提交
						...
						editDetailCases(params).then(res => {
			...
			addSubmit(formName) {
				this.$refs[formName].validate(valid => {
					if (valid) {
						...
						addDetailCases(params).then(res => {
